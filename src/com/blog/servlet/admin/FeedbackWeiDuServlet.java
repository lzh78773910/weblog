...
public class FeedbackWeiDuServlet extends HttpServlet {
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		int totalCount = iFeedbackService.getTotalCountByWeiDu(a_id);//总数据数
		page.setTotalCount(totalCount);
	    int pageSize=8;
	    int totalPage =totalCount%pageSize==0?totalCount/pageSize:totalCount/pageSize+1;
	    page.setTotalPage(totalPage);
	    List<Feedback> list = iFeedbackService.feedbackWeiDu(a_id, currentPage, pageSize);
	    page.setFeedback(list);
	    
	    request.setAttribute("page", page);
        request.getRequestDispatcher("/admin/a_feedbackwewidu.jsp").forward(request, response);
